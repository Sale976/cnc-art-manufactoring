    ...
    if not st.session_state.logged_in:
        ...
        if st.button("Sačuvaj i objavi 🚀"):
            if novi_naziv and otpremljen_fajl:
                ...
                # 1. Prvo je sačuvamo lokalno da bi se odmah videla
                fajl_bajtovi = otpremljen_fajl.getbuffer()
                with open(putanja_za_cuvanje, "wb") as f:
                    f.write(fajl_bajtovi)
                
                # 2. AUTOMATSKO SLANJE NA GITHUB REPOZITORIJUM
                try:
                    # Podatke povlačimo iz Streamlit Secrets-a radi bezbednosti (podešava se na Streamlit Cloud-u)
                    GITHUB_TOKEN = st.secrets["GITHUB_TOKEN"]
                    REPO_NAME = st.secrets["GITHUB_REPO"]  # npr. "tvoje_ime/cnc-galerija"
                    ...
                    # Šaljemo fajl direktno na GitHub granu (main)
                    repo.create_file(
                        path=github_putanja,
                        message=f"Dodat novi CNC projekat: {novi_naziv}",
                        content=bytes(fajl_bajtovi),
                        branch="main"
                    )
                    st.success(f"Projekat je uspešno dodat i sinhronizovan sa GitHub-om! Kroz par minuta biće trajno sačuvan.")
                    st.rerun()
                except Exception as e:
                    # Ako nisi podesio Secrets, aplikacija će i dalje raditi lokalno ali uz upozorenje
                    st.warning(f"Slika je privremeno sačuvana, ali sinhronizacija sa GitHub-om nije uspela. Podesi st.secrets! (Greška: {e})")
            else:
